...
def find_scores_for_date_dict(month, day, year):
    ...
    for game in games:
        ...
        z=z+1
    return all_games
def find_ptscores_dict(pts):
    #print now.year, now.month, now.day, now.hour, now.minute, now.second
    thedate = datetime.datetime.now()
    z = 0
    while str(thedate.year) > '1984':
        print thedate
        scoreboard = nba_py.Scoreboard(thedate.month, thedate.day, thedate.year)
        line_score_list = scoreboard.line_score()
        #print line_score_list
        number_of_games = (len(line_score_list))/2
        #print str(number_of_games) + " games on this day"
        ...
        all_games = {}
        for game in games:
            box = nba_py.game.Boxscore(str(game))
            #print box.game_summary()
            all_pts = box.player_stats()['PTS']
            box2 = nba_py.game.BoxscoreSummary(str(game))
            for pt in all_pts:
                if pt > pts:
                    box2 = box2.line_score()
                    box2 = box2.values.tolist()
                    hometeam = box2[0][5]
                    awayteam = box2[1][5]
                    hometeamscore = box2[0][22]
                    awayteamscore =  box2[1][22]
                    game_string = awayteam + " " + str(awayteamscore) + " " + hometeam + " " + str(hometeamscore)
                    all_games[game] = game_string
                    print game_string
...
def index(request):
    ...
    for stat, player in top_ppg_list:
        ppg_list_sentences[i] = (str(player) + " : " + str(stat))
        i = i + 1
    #for stat, player in top_ppg_list:
    ...
    i = 0
    for stat, player in top_ppg_list:
        the_name = "player/" + str(player)
        the_name = the_name.replace(' ', '')
        print the_name
        player_namess[i] = the_name
...
def player(request, player_name_url):
    ...
    if capitalLetters == 0:
        ...
        season = pandas.read_json(player.seasonStats, typ='frame', orient='records')
    else:
        player = Player.objects.get(combinedName=player_name)
        player1 = eval(player.PlayerCareerStats)
        player2 = eval(player.personalInfo)
        season = pandas.read_json(player.seasonStats, typ='frame', orient='records')
    season.pop("LEAGUE_ID")
    season.pop("PLAYER_ID")
    season.pop("TEAM_ID")
    #frame = frame[[]]
    ...
    print "HAHASHAHSDHASDH"
    season.sort_index(axis=0,ascending=True, inplace=True)
    print season
    season = season.to_html()
    #stats = pandas.read_json(seasonalstats, typ='frame', orient='records')
    #print player
    ...
    season = season.replace('<table border="1" class="dataframe">', "<table class='table table-striped table-hover '>")
    #print season

    context_dict['seasons'] = season
